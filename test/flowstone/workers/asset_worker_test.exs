defmodule FlowStone.Workers.AssetWorkerTest do
  ...
  defmodule OptionalPipeline do
    use FlowStone.Pipeline

    asset :optional_target do
      depends_on([:optional_dep])
      optional_deps([:optional_dep])
      execute fn _, deps -> {:ok, deps[:optional_dep] || :missing} end
    end
  end
  ...
  setup do
    ...
    FlowStone.register(OptionalPipeline, registry: :worker_registry)
  ...
  test "executes when only optional deps are missing", %{io_opts: io_opts, run_config: run_config} do
    job = %Oban.Job{
      args: %{
        "asset_name" => "optional_target",
        "partition" => Partition.serialize(:p2),
        "run_id" => Ecto.UUID.generate(),
        "use_repo" => false
      }
    }
    ...
    assert :ok = AssetWorker.perform(job, run_config)
    assert {:ok, :missing} = FlowStone.IO.load(:optional_target, :p2, io_opts)
  end
